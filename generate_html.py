...
MAIN_URL = "https://apnetv.xyz/Hindi-Serial/episodes/Bigg-Boss-Season-20 [apnetv.xyz]"
HEADERS = {
...
match = re.search(
r'engine\.getProxiedUrl\(\s*["\'](https?://[^"\']+\.mp4)["\']\s*\)',
res.text,
)
...
episode_data.append({"title": title, "mp4": mp4})
...
.item {{ padding: 15px; background: #1e1e1e; border-radius: 6px; cursor: pointer; transition: background 0.2s; display: flex; justify-content: space-between; align-items: center; }}
.item:hover {{ background: #2a2a2a; }}
.item.active {{ border-left: 4px solid #e50914; background: #252525; }}
a.direct-link {{ color: #888; text-decoration: none; font-size: 0.9em; border: 1px solid #444; padding: 4px 8px; border-radius: 4px; }}
a.direct-link:hover {{ color: #fff; border-color: #666; }}
</style>
</head>
<body>
<div class="container">
...
<span>{ep['title']}</span>
<a class="direct-link" href="{ep['mp4']}" target="_blank" onclick="event.stopPropagation()">Direct MP4</a>
</div>"""
